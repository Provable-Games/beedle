...
pub fn get_twamm_extension_address(chain_id: felt252) -> ContractAddress {
    if chain_id == MAINNET_CHAIN_ID {
        contract_address_const::<0x043e4f09c32d13d43a880e85f69f7de93ceda62d6cf2581a582c6db635548fdc>()
    } else if chain_id == SEPOLIA_CHAIN_ID {
        contract_address_const::<0x073ec792c33b52d5f96940c2860d512b3884f2127d25e023eb9d44a678e4b971>()
    } else {
        panic!("unsupported chain")
    }
}
...
pub fn get_positions_address(chain_id: felt252) -> ContractAddress {
    if chain_id == MAINNET_CHAIN_ID {
        ...
        contract_address_const::<0x04afc78d6fec3b122fc1f60276f074e557749df1a77a93416451be72c435120f>()
    } else {
        panic!("unsupported chain")
    }
}
...
pub fn get_registry_address(chain_id: felt252) -> ContractAddress {
    if chain_id == MAINNET_CHAIN_ID {
        contract_address_const::<0x064bdb4094881140bc39340146c5fcc5a187a98aec5a53f448ac702e5de5067e>()
    } else if chain_id == SEPOLIA_CHAIN_ID {
        contract_address_const::<0x04484f91f0d2482bad844471ca8dc8e846d3a0211792322e72f21f0f44be63e5>()
    } else {
        panic!("unsupported chain")
    }
}
